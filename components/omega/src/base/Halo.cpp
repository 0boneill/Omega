...
HaloD *HaloD::DefaultHalo = nullptr;
std::map<std::string, std::unique_ptr<HaloD>> HaloD::AllHalos;
...
// Empty constructor for ExchList class
HaloD::ExchListD::ExchListD() = default;
...
HaloD::NeighborD::NeighborD(
   ...
   // Construct the member ExchList objects
   SendLists[0] = ExchListD(SendCell);
   SendLists[1] = ExchListD(SendEdge);
   SendLists[2] = ExchListD(SendVrtx);

   RecvLists[0] = ExchListD(RecvCell);
...
int HaloD::packBuffer(const HostArray1DI4 Array) {
...
int HaloD::unpackBuffer(HostArray1DI4 &Array) {
